...
class PostProcessRules:
    ...
    def morph_energy_reliability_fusion(
        ...
        层参数覆盖机制：
          layer_params 的 key 为层名（与 energy_layers 对应），value 为该层的参数字典。
          每层的连通域分析会优先使用 layer_params 中该层的专属值，未指定的参数
          退化为全局默认值。例如：
            layer_params = {
                "energy_layer1": {"small_area_threshold": 2000, "area_min": 100},
                "energy_layer2": {"small_ratio_max": 0.4},
            }
          则 energy_layer1 的 small_area_threshold=2000, area_min=100，其余参数用全局默认值；
          energy_layer2 的 small_ratio_max=0.4，其余参数用全局默认值；
          energy_layer3 全部使用全局默认值。
        ...
        参数：
            maps: 输入的 map 字典
            energy_layers: 要处理的能量层名称元组
            area_min: 连通域最小面积阈值（全局默认值）
            small_area_threshold: 小连通域面积阈值（全局默认值）
            small_ratio_max: 小连通域占比上限（全局默认值）
            total_area_min_ratio: mask 占图面积比下限（全局默认值）
            total_area_max_ratio: mask 占图面积比上限（全局默认值）
            connectivity: 连通域连通性（4/8）
            remove_small_components: 是否移除小连通域
            layer_params: 逐层参数覆盖字典，key 为层名，value 为该层的参数字典
        """
        result = {k: v.copy() for k, v in maps.items()}
        ...
        for layer in energy_layers:
            per = lp.get(layer, {})  # 该层专属参数，未指定则空字典
    ...
    def reliability_calibrate(
        ...
        本规则依赖规则13在 maps 中留下的 energy_reliability_dict（q 值字典），
        无需重复计算连通域。q=1 的层参与融合，q=0 的层直接丢弃。

        参数：
            maps: 包含能量图/能量差图的字典，必须含有 energy_reliability_dict（由规则13生成）
            recon_key: 重构误差图的 key，默认 "recon_error"
        ...
        if args.apply_pipeline:
            ...
            for i, rule in enumerate(args.rule):
                ...
                if rule == "clip":
                    ...
                    # 从参数块中分离 "层名.参数=值" 语法，组装 layer_params
                    layer_params: Dict[str, Dict] = {}
                    global_params: Dict[str, Any] = {}
                    known_layers = ("energy_layer1", "energy_layer2", "energy_layer3")
                    known_param_names = (
                        "area_min", "small_area_threshold", "small_ratio_max",
                        "total_area_min_ratio", "total_area_max_ratio", "connectivity",
                    )
                    for k, raw_v in p.items():
                        if "." in k:
                            prefix, param_name = k.rsplit(".", 1)
                            if prefix in known_layers and param_name in known_param_names:
                                try:
                                    v = int(raw_v)
                                    ...
                                    try:
                                        ...
                                        v = raw_v
                                layer_params.setdefault(prefix, {})[param_name] = v
                                continue
                        global_params[k] = raw_v

                    gp = global_params
                    processed = PostProcessRules.morph_energy_reliability_fusion(
                        processed,
                        area_min=int(gp.get("area_min", 50)),
                        small_area_threshold=int(gp.get("small_area_threshold", 50)),
                        small_ratio_max=float(gp.get("small_ratio_max", 0.6)),
                        total_area_min_ratio=float(gp.get("total_area_min_ratio", 0.001)),
                        total_area_max_ratio=float(gp.get("total_area_max_ratio", 0.35)),
                        connectivity=int(gp.get("connectivity", 8)),
                        remove_small_components=gp.get("remove_small_components", "True").lower() == "true",
                    ...
                    for layer, info in rel_dict.items():
                        ...
                        p_used = info.get("params", {})
                        print(f"    {layer}: q={q:.1f} | {status}")
                        print(f"      params used: area_min={p_used.get('area_min','?')}, "
                              f"small_area_threshold={p_used.get('small_area_threshold','?')}, "
                              f"small_ratio_max={p_used.get('small_ratio_max','?')}")
                        print(f"      CC={info.get('total_components', 0)}, max_area={info.get('max_area', 0)}, "
                              f"small_ratio={info.get('small_ratio', 0):.3f}, "
                              f"area_ratio={info.get('total_area_ratio', 0):.4f}")
                elif rule == "reliability_calibrate":
                    processed = PostProcessRules.reliability_calibrate(
                        processed,
                        recon_key=p.get("recon_key", "recon_error"),
                        lambda_enhance=float(p.get("lambda_enhance", 0.25)),
                        lambda_suppress=float(p.get("lambda_suppress", 0.0)),
                        use_suppression=p.get("use_suppression", "False").lower() == "true",
                        use_energy_diff=p.get("use_energy_diff", "True").lower() == "true",
                        energy_layers=(
                            p.get("energy_layers", "energy_layer1,energy_layer2,energy_layer3")
                        ).split(","),
                    )
                    calibrated = processed.get("recon_error_calibrated")
                    gain = processed.get("calibration_gain")
                    rel_mask = processed.get("energy_reliability_mask")
                    print(f"  [规则14] 校准完成:")
                    print(f"    lambda_enhance={p.get('lambda_enhance', 0.25)}, "
                          f"lambda_suppress={p.get('lambda_suppress', 0.0)}, "
                          f"use_suppression={p.get('use_suppression', 'False')}, "
                          f"use_energy_diff={p.get('use_energy_diff', 'True')}")
                    if calibrated is not None:
                        print(f"    calibrated: mean={float(calibrated.mean()):.4f}, "
                              f"std={float(calibrated.std()):.4f}, max={float(calibrated.max()):.4f}")
                    if gain is not None:
                        print(f"    gain: min={float(gain.min()):.4f}, max={float(gain.max()):.4f}")
                    if rel_mask is not None:
                        nonzero = int(np.count_nonzero(rel_mask))
                        total = rel_mask.size
                        print(f"    reliability_mask coverage: {nonzero}/{total} "
