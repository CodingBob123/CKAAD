...
class CoordAtt(nn.Module):
    def __init__(self, inp, oup, reduction=32):
        ...
        self.pool_h = nn.AdaptiveAvgPool2d((None, 1))  # 垂直方向池化层
        self.pool_w = nn.AdaptiveAvgPool2d((1, None))  # 水平方向池化层

        # mip = max(8, inp // reduction)

        # self.conv1 = nn.Conv2d(inp, mip, kernel_size=1, stride=1, padding=0)
        self.conv1 = nn.Conv2d(inp, oup, kernel_size=1, stride=1, padding=0, bias=True)
