...
class StaticAlignmentBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, stride=2, branch_type='feature1', norm_layer=nn.InstanceNorm2d):
        ...
        # 1. 残差模块准备，为了能够进行残差连接，我们需要一个Shortcut路径来调整原始输入的尺寸/通道
        # 如果 stride > 1 或者 in != out，需要下采样原始输入
        ...
        if branch_type == 'feature1':
            # Feature1: 纹理感知的对齐
            self.alignment = self._build_detail_preserving_alignment(in_channels, out_channels, stride, norm_layer)
    ...
    def _build_detail_preserving_alignment(self, in_channels, out_channels, stride, norm_layer):
        """
        [优化] Feature1: 纹理感知
        改动: 使用标准卷积，移除会导致尺寸变化的空洞卷积
        ...
        return nn.Sequential(
            ...
            nn.ReLU(inplace=True),
            # 深度可分离卷积增强纹理感知
            nn.Conv2d(out_channels//2, out_channels//2, 3, stride=1, padding=1, groups=out_channels//2, bias=False),
