...
# ======================================================================
# 推荐参数配置说明（基于 UnifiedAnomalyController 的固定概率策略）
# ======================================================================
#
# 异常合成模块              | 参数/模式                      | 推荐值       | 说明
# --------------------------|--------------------------------|--------------|------------------------------------------
# [控制器] UnifiedAnomaly   | --anomaly_strategy             | prob         | 固定概率: 正常50% + Perlin30% + Pixel20%
...
# [像素] 每样本像素异常概率  | --pixel_anomaly_prob            | 0.20         | 20% 样本走像素级异常
...
# [Perlin] 每样本 Perlin 概率| --perlin_anomaly_prob           | 0.30         | 30% 样本走 Perlin 特征扰动
...
# === 控制器策略 ===
# 'prob'        : 固定概率分配（推荐用于验证性能退化是否来自 progressive 动态配比）
...
do
    ...
    *)
    ...
    case $normal in 'cable' | 'toothbrush'|'transistor'|'wood')
            enable_enhance_flag="--enable_enhancement"
            ;;
        *)
            enable_enhance_flag=""
            ;;
